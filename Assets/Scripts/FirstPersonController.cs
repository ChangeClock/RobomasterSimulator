...
namespace StarterAssets
{
	...
	public class FirstPersonController : NetworkBehaviour
	{
		...
		public delegate void ShootAction(Vector3 userPosition, Quaternion userDirection, Vector3 ShootVelocity);
		...
		private void Shoot()
		{
			if (_input.shoot && _shootTimeoutDelta <= 0.0f && ShooterEnabled) 
			{
				...
				// Debug.Log("Shoot");
				if (OnShoot != null){
					// Vector3 _shootOffset = new Vector3(transform.rotation.x, transform.rotation.y, transform.rotation.z);
					// Vector3 _shootOffset = (new Vector3 (1, 0, 0) - transform.position).normalized;
					
					Vector3 _shootOffset = Vector3.zero;
					_shootOffset = transform.forward;
					_shootOffset.y += ShootHeight;
					...
					OnShoot(transform.position + _shootOffset, transform.rotation, transform.forward * ShootSpeed);
				}
			}
